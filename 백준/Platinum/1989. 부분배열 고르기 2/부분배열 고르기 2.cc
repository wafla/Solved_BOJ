...
pair<ll, pair<ll,ll>> Div(int left, int right)
{
	if (left == right)
		return { arr[left] * arr[left], {left, left} };
	auto cur = solve(1, 1, n, left, right);
	pair<ll, pair<ll, ll>> ans = { cur.X * arr[cur.Y],{left, right} };
	if (cur.Y - 1 >= left)
...
int main()
{
	...
	tree.resize(n * 4);
	init(1, 1, n);
	pair<ll, pair<ll, ll>> ans = Div(1, n);
	cout << ans.X << '\n';
